...
//contador
$totalCategoria = $connection->prepare("SELECT COUNT(idCategoria) as total from tbCategoria");
$totalCategoria->execute();
$total = $totalCategoria->fetchAll(PDO::FETCH_ASSOC);
$abc = $total[0]['total'];


while ($row = $stmt->fetch(PDO::FETCH_BOTH)) {
    $categorie .= $row['nomeCategoria'] . "<br />";
}
        ...
        <table style='width: 100%; text-align: center; border-collapse: collapse;'>
            ...
            <tr>
                <td style='border: 2px solid white; border-collapse: collapse; padding: 10px; color: #202020; background-color: #90b4ed;'>
                    <h3 style='margin:10px 0';> $categorie </h3>
                </td>
            </tr>
        </table>
    </div>
